...
import { View, Text, StyleSheet, TouchableOpacity, Animated, Dimensions, ScrollView, Switch, TextInput } from 'react-native';
import { Ionicons, Entypo } from '@expo/vector-icons';
import Svg, { Path } from 'react-native-svg';

const { width } = Dimensions.get('window');

// --- CÔNG THỨC TOÁN HỌC (Đồng bộ y hệt HomeScreen) ---
...
// --- CẬP NHẬT COMPONENT CARD (CHỐNG GIẬT LAG KHI GÕ) ---
const SettingCard = ({ title, value, onChangeValue, onIncrease, onDecrease }) => {
  // Tạo một bộ đệm cục bộ dạng chuỗi (String) để chứa giá trị đang gõ
  const [textVal, setTextVal] = React.useState(String(value));

  // Lắng nghe sự thay đổi nếu người dùng bấm nút [+] hoặc [-]
  React.useEffect(() => {
    if (parseInt(textVal || '0') !== value) {
    ...
    <View style={styles.cardContainer}>
      ...
      <View style={styles.cardControls}>
        ...
        <TextInput 
          ...
          value={textVal} // Sử dụng bộ đệm thay vì giá trị gốc
          keyboardType="numeric"
          maxLength={3}
          onChangeText={(text) => {
            // Lọc bỏ các ký tự không phải là số
            const cleanText = text.replace(/[^0-9]/g, '');
            
            // Cập nhật bộ đệm lập tức để gõ siêu mượt, không bị giật số
            setTextVal(cleanText); 
            
            // Cập nhật ngầm vào hệ thống gốc
            onChangeValue(cleanText === '' ? 0 : parseInt(cleanText));
          }}
          onBlur={() => {
            // Khi người dùng bấm ra ngoài (bỏ chọn ô nhập), ta mới "dọn dẹp" các số 0 thừa
            setTextVal(String(value));
          }}
        />
...
export default function SetTimeScreen({ route, navigation }) {
  const [activeTab, setActiveTab] = useState(0); 
  const slideAnim = useRef(new Animated.Value(0)).current; 

  const [studyTime, setStudyTime] = useState(route.params?.studyTime ?? 25);
  const [shortBreak, setShortBreak] = useState(route.params?.shortBreak ?? 5);
  const [longBreak, setLongBreak] = useState(route.params?.longBreak ?? 15);
  const [cycle, setCycle] = useState(route.params?.cycle ?? 4);
  const [isFlipEnabled, setIsFlipEnabled] = useState(route.params?.isFlipEnabled ?? true);
...
<ScrollView contentContainerStyle={styles.scrollContent}>
        <SettingCard 
          title="Thời gian học" 
          value={studyTime} 
          onChangeValue={setStudyTime} // <--- THÊM DÒNG NÀY
          onIncrease={() => setStudyTime(s => s + 1)} 
          onDecrease={() => setStudyTime(s => Math.max(1, s - 1))} 
        />
        <SettingCard 
          title="Nghỉ ngắn" 
          value={shortBreak} 
          onChangeValue={setShortBreak} // <--- THÊM DÒNG NÀY
          onIncrease={() => setShortBreak(s => s + 1)} 
          onDecrease={() => setShortBreak(s => Math.max(1, s - 1))} 
        />
        <SettingCard 
          title="Nghỉ dài" 
          value={longBreak} 
          onChangeValue={setLongBreak} // <--- THÊM DÒNG NÀY
          onIncrease={() => setLongBreak(s => s + 1)} 
          onDecrease={() => setLongBreak(s => Math.max(1, s - 1))} 
        />
        <SettingCard 
          title="Chu Kỳ" 
          value={cycle} 
          onChangeValue={setCycle} // <--- THÊM DÒNG NÀY
      ...
      {/* --- BOTTOM BAR ĐỒNG BỘ VỚI HOMESCREEN --- */}
      <View style={styles.bottomBarWrapper}>
        ...
        <Animated.View style={[
          ...
          <Svg width={110} height={60} viewBox="0 0 110 60">
            <Path 
              ...
              fill="#F3EBE9" // Màu chữ U trùng với màu nền của màn hình
            />
          </Svg>
        </Animated.View>
        ...
        <View style={styles.iconsContainer}>
          {TABS.map((tab, index) => {
            ...
            // Đồng bộ hiệu ứng nảy (translateY)
            ...
            // Đồng bộ hiệu ứng thu phóng (scale) giống HomeScreen
            const scale = slideAnim.interpolate({
              inputRange: [index - 1, index, index + 1],
              outputRange: [0.75, 1.2, 0.75], // Không chọn thì mờ 0.75, chọn thì bự 1.2
              extrapolate: 'clamp',
            });
...
const styles = StyleSheet.create({
  ...
  container: { flex: 1, backgroundColor: '#F3EBE9' },
  header: { 
    flexDirection: 'row', 
    alignItems: 'center', 
    paddingTop: 60, 
  ...
  minusButton: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#F2CCCA', justifyContent: 'center', alignItems: 'center' },
  plusButton: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#67C3BD', justifyContent: 'center', alignItems: 'center' },
  valueBox: { flex: 1, height: 46, borderWidth: 1.5, borderColor: '#67C3BD', borderRadius: 10, marginHorizontal: 15, justifyContent: 'center', alignItems: 'center' },
  valueText: { fontSize: 18, fontWeight: 'bold', color: '#2A3B38' },
  
  // --- ĐỒNG BỘ STYLE BOTTOM BAR ---
  ...
  // Cutout mask lấy 100% y hệt HomeScreen (nhưng đổi backgroundColor thành màu nền SetTime)
  cutoutMask: {
    position: 'absolute',
    bottom: 35, 
    left: 0, 
    ...
    backgroundColor: '#F3EBE9', // <--- Điểm mấu chốt để nó hòa vào nền
    zIndex: 1,
  },
