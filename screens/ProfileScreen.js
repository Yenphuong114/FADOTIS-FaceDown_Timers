...
import * as ImagePicker from 'expo-image-picker'; // Thư viện lấy ảnh

const { width } = Dimensions.get('window');

// --- CÔNG THỨC BOTTOM BAR ---
...
export default function ProfileScreen({ navigation }) {
  // Màn hình Profile là icon số 4 (index = 3)
  const [activeTab, setActiveTab] = useState(3); 
  const slideAnim = useRef(new Animated.Value(3)).current; 

  // --- LOGIC ĐỔI AVATAR TỪ MÁY ---
  // Khởi tạo ảnh mặc định (bạn có thể thay bằng URI ảnh rỗng null nếu muốn)
  const [avatar, setAvatar] = useState('https://api.dicebear.com/7.x/notionists/png?seed=User&backgroundColor=e6e6e6');

const pickImage = async () => {
    let result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ['images'], // <--- ĐỔI THÀNH NHƯ THẾ NÀY LÀ XONG
      allowsEditing: true, 
      aspect: [1, 1], 
      quality: 1, 
    });
    ...
    if (!result.canceled) {
      setAvatar(result.assets[0].uri); // Lưu lại đường dẫn ảnh mới
    }
  };

  // --- LOGIC THANH ĐIỀU HƯỚNG ---
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{ marginTop: -70, zIndex: 1, elevation: 10 }} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        
        {/* 2. AVATAR NẰM ĐÈ LÊN HEADER VÀ BODY */}
        <View style={styles.avatarContainer}>
          ...
          <Text style={styles.userName}>Lạc Hồng Student</Text>
          <View style={styles.badgeLabel}>
            <Text style={styles.badgeText}>Chuyên gia tập trung</Text>
          </View>
        </View>

        {/* 3. KHU VỰC THÀNH TỰU (ACHIEVEMENTS) */}
        <Text style={styles.sectionTitle}>Thành tựu</Text>
        <View style={styles.achievementsRow}>
          
          <View style={styles.achievementItem}>
            <View style={[styles.achievementIconBox, { backgroundColor: '#F3EBE9' }]}>
              <Ionicons name="time" size={32} color="#469E6F" />
            </View>
            <Text style={styles.achievementName}>Chuyên gia tập trung</Text>
          </View>

          <View style={styles.achievementItem}>
            <View style={[styles.achievementIconBox, { backgroundColor: '#F3EBE9' }]}>
              <Ionicons name="book" size={30} color="#469E6F" />
            </View>
            <Text style={styles.achievementName}>Học tập sự tuần</Text>
          </View>

          <View style={styles.achievementItem}>
            <View style={[styles.achievementIconBox, { backgroundColor: '#F3EBE9' }]}>
              <Ionicons name="hourglass" size={30} color="#469E6F" />
            </View>
            <Text style={styles.achievementName}>Học giả mới</Text>
          </View>
        ...
        <Animated.View style={[
          ...
          <Svg width={110} height={60} viewBox="0 0 110 60">
            <Path 
              ...
              fill="#F3EBE9" // Khớp màu nền Profile
            />
          </Svg>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  // 1. HEADER XANH
  headerGreen: {
    backgroundColor: '#469E6F',
    height: 180,
    borderBottomLeftRadius: 40, // Bo cong dưới cùng trái
    borderBottomRightRadius: 40,
    paddingTop: 60,
    paddingHorizontal: 20,
  },
  headerTop: { width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  ...
  avatarContainer: {
    alignItems: 'center',
     // Đẩy avatar lún ngược lên phần màu xanh
    marginBottom: 30,
  },
  avatarWrapper: {
    position: 'relative',
  ...
  avatarImage: {
    ...
    borderWidth: 5, borderColor: '#F3EBE9', // Viền bo tròn màu kem tiệp màu nền
    backgroundColor: '#fff'
  },
  cameraIconBadge: {
    position: 'absolute', bottom: 5, right: 5,
    backgroundColor: '#469E6F', width: 34, height: 34, borderRadius: 17,
    justifyContent: 'center', alignItems: 'center',
    borderWidth: 3, borderColor: '#F3EBE9'
  },
  userName: { fontSize: 26, fontWeight: 'bold', color: '#2A3B38', marginTop: 15 },
  badgeLabel: {
    backgroundColor: '#BEE3E0', paddingHorizontal: 15, paddingVertical: 5, borderRadius: 15, marginTop: 8
  },
  badgeText: { color: '#3D544C', fontWeight: 'bold', fontSize: 14 },

  // 3. THÀNH TỰU
  sectionTitle: { fontSize: 20, fontWeight: 'bold', color: '#2A3B38', marginBottom: 20 },
  achievementsRow: {
  ...
  achievementName: { fontSize: 13, color: '#3D544C', textAlign: 'center', fontWeight: 'bold', paddingHorizontal: 5 },

  // 4. LOGOUT
  logoutButton: {
    backgroundColor: '#fff', paddingVertical: 15, borderRadius: 15, alignItems: 'center',
    borderWidth: 1, borderColor: '#D6CBC9', marginHorizontal: 20,
  },
  logoutText: { color: '#E57373', fontSize: 18, fontWeight: 'bold' },
