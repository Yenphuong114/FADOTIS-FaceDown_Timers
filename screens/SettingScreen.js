...
import { View, Text, StyleSheet, TouchableOpacity, Animated, Dimensions, ScrollView, Switch } from 'react-native';
...
// --- KHAI BÁO 4 ICON SVG (Dùng chung cho Bottom Bar) ---
const SvgTomato = () => (
...
export default function SettingScreen({ navigation }) {
  // Setting vẫn nằm trong tab Profile nên để activeTab = 3
  const [activeTab, setActiveTab] = useState(3); 
  const slideAnim = useRef(new Animated.Value(3)).current; 
  
  // State quản lý nút Gạt chế độ
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 2. KHU VỰC NỘI DUNG MÀU TRẮNG BO GÓC TRÊN */}
      <View style={styles.whiteBody}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          
          {/* Item 1: Thống kê thời gian */}
          <TouchableOpacity style={styles.settingItem}>
            <View style={styles.iconBox}>
              <Ionicons name="time" size={24} color="#1a1a1a" />
            </View>
            <Text style={styles.settingText}>Thống kê thời gian</Text>
          ...
          <TouchableOpacity style={styles.settingItem}>
            ...
            <Text style={styles.settingText}>Lịch sử học</Text>
            <Ionicons name="chevron-forward" size={20} color="#666" />
          </TouchableOpacity>
          ...
          <TouchableOpacity style={styles.settingItem}>
            ...
            <Text style={styles.settingText}>Đổi tên</Text>
            <Ionicons name="chevron-forward" size={20} color="#666" />
          </TouchableOpacity>

          {/* Item 3: Chế độ (Kèm nút gạt Switch) */}
          <View style={styles.settingItem}>
            <View style={styles.iconBox}>
              <Ionicons name="contrast" size={24} color="#1a1a1a" />
            </View>
            <Text style={styles.settingText}>Chế độ</Text>
            <Switch
              trackColor={{ false: "#D6CBC9", true: "#469E6F" }}
              thumbColor="#FFF"
              value={isDarkMode}
          ...
          <TouchableOpacity style={styles.settingItem}>
            <View style={styles.iconBox}>
              <Ionicons name="notifications-sharp" size={24} color="#1a1a1a" />
            </View>
            <Text style={styles.settingText}>Thông báo</Text>
            <Ionicons name="chevron-forward" size={20} color="#666" />
          </TouchableOpacity>

          {/* Đệm đẩy lên cho khỏi vướng Bottom Bar */}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#469E6F' }, // Để nền màu xanh khớp với header
  ...
  headerGreen: {
    ...
    alignItems: 'center', // Căn giữa chữ SETTING
  },
  backBtn: {
    position: 'absolute',
    left: 20,
  ...
  headerTitle: { 
    fontSize: 30, 
    fontWeight: '900', 
    color: '#2A3B38', // Màu tối giống trong thiết kế
    marginTop: 35, // Đẩy chữ SETTING xuống dưới mũi tên
  },

  // 2. KHU VỰC TRẮNG BO GÓC BÊN DƯỚI
  whiteBody: {
    flex: 1,
    backgroundColor: '#FFF',
    borderTopLeftRadius: 35,
    borderTopRightRadius: 35,
    marginTop: -20, // Kéo lùi lên để viền bo góc nằm đè lên nền xanh mượt mà
    overflow: 'hidden',
  },
  scrollContent: {
    paddingTop: 40,
