...
import { Calendar, LocaleConfig } from 'react-native-calendars'; // Thư viện lịch
...
export default function CalendarScreen({ navigation }) {
  // Lịch là icon số 3 (index = 2)
  const [activeTab, setActiveTab] = useState(2); 
  const slideAnim = useRef(new Animated.Value(2)).current; 

// --- BƯỚC 1: TỰ ĐỘNG LẤY NGÀY HÔM NAY TỪ HỆ THỐNG ---
  const today = new Date();
  const todayString = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;

  // Cho ngày mặc định đang chọn là ngày hôm nay luôn (Mở lịch lên là xem hôm nay)
  const [selectedDate, setSelectedDate] = useState(todayString); 

  // Giả lập danh sách những ngày bạn đã học (sẽ có chấm xanh)
  const studiedDates = ['2026-05-18', '2026-05-10', '2026-05-15']; 

  // --- BƯỚC 2: NHÀO NẶN GIAO DIỆN CÁC NGÀY ---
// --- BƯỚC 2: NHÀO NẶN GIAO DIỆN CÁC NGÀY (KHÔNG DÙNG VIỀN) ---
  const getMarkedDates = () => {
    let marks = {};

    // 1. Quét qua những ngày đã học và gắn dấu chấm (dot)
    studiedDates.forEach(date => {
      marks[date] = {
        marked: true, 
        customStyles: {
          container: { }, // Để trống để không có nền
          text: { color: '#2A3B38' }
        }
      };
    });

    // 2. Gắn nền tròn màu hồng cho "Ngày hôm nay"
    marks[todayString] = {
      ...marks[todayString],
      customStyles: {
        container: { backgroundColor: '#F2D1CE', borderRadius: 20 }, 
        text: { color: '#2A3B38', fontWeight: 'bold' }
      }
    };

    // 3. Đánh dấu ngày đang click chọn (BỎ VIỀN)
    // Nếu ngày đang click không phải là ngày hôm nay, ta cho nó một nền tròn xanh nhạt
      ...
      marks[selectedDate] = {
        ...
        customStyles: {
          container: { 
            ...
            backgroundColor: '#D1EBE9', // Nền xanh ngọc nhạt thay cho viền
            borderRadius: 20 
          },
          text: { color: '#2A3B38', fontWeight: 'bold' }
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.calendarWrapper}>
        <Calendar
          // Giao diện Lịch (Theme) y hệt thiết kế
          theme={{
            backgroundColor: '#F3EBE9',
            calendarBackground: '#F3EBE9',
            textSectionTitleColor: '#2A3B38', // Màu chữ Sun, Mon...
            selectedDayBackgroundColor: 'transparent',
            selectedDayTextColor: '#2A3B38',
            todayTextColor: '#2A3B38',
            dayTextColor: '#2A3B38',
            textDisabledColor: '#96C6C2', // Màu ngày của tháng trước/sau (xanh nhạt)
            dotColor: '#67C3BD', // Màu chấm tròn đã học
            selectedDotColor: '#67C3BD', 
            arrowColor: '#67C3BD', // Màu mũi tên < >
            monthTextColor: '#67C3BD', // Màu chữ Tháng
            textMonthFontWeight: 'bold',
            textDayHeaderFontWeight: 'bold',
            textMonthFontSize: 20,
          }}
          // Cho phép dùng CustomStyle ở trên
          markingType={'custom'}
          markedDates={getMarkedDates()}
          onDayPress={day => {
            setSelectedDate(day.dateString);
          }}
          // Hiển thị ngày tháng theo thiết kế của bạn
...
const styles = StyleSheet.create({
  ...
  divider: { height: 1, backgroundColor: '#D6CBC9', width: '100%', marginBottom: 20 },
  
  // Khung viền bọc quanh bộ lịch
  calendarWrapper: {
    marginHorizontal: 20,
    borderWidth: 1,
    borderColor: '#67C3BD',
