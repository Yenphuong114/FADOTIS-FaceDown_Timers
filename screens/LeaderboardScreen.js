...
// --- CÔNG THỨC TOÁN HỌC CHO BOTTOM BAR ---
const PADDING_HORIZONTAL = 30; 
const TAB_WIDTH = (width - PADDING_HORIZONTAL * 2) / 4; 
const MASK_SIZE = 90; 
...
export default function LeaderboardScreen({ navigation }) {
  // Mặc định ở màn hình này Tab số 1 (Leaderboard) sẽ được sáng lên
  const [activeTab, setActiveTab] = useState(1); 
  const slideAnim = useRef(new Animated.Value(1)).current; // Giá trị khởi tạo là 1 thay vì 0

  // Chuyển đổi giữa "Tuần này" và "Tháng này"
  ...
  const handleTabPress = (index) => {
    ...
    Animated.timing(slideAnim, {
      toValue: index,
      duration: 200, // Thời gian trượt siêu nhanh (150 mili-giây)
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Bảng xếp hạng động lực</Text>
      </View>
      <View style={styles.divider} />

      {/* KHU VỰC CUỘN ĐỂ KHÔNG CHẠM BOTTOM BAR */}
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        
        {/* THANH ĐỔI TAB (Tuần này / Tháng này) */}
        <View style={styles.toggleContainer}>
          <TouchableOpacity 
            style={[styles.toggleButton, timeFilter === 'week' && styles.toggleActive]}
            onPress={() => setTimeFilter('week')}
          >
            <Text style={[styles.toggleText, timeFilter === 'week' && styles.toggleTextActive]}>Tuần này</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.toggleButton, timeFilter === 'month' && styles.toggleActive]}
            onPress={() => setTimeFilter('month')}
          >
            <Text style={[styles.toggleText, timeFilter === 'month' && styles.toggleTextActive]}>Tháng này</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.podiumContainer}>
          ...
          <View style={[styles.podiumItem, styles.podiumRank2]}>
            <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=Dao&backgroundColor=f2d1ce' }} style={styles.avatar} />
            <Text style={styles.podiumName}>Đào</Text>
            <Text style={styles.podiumRankText}>Hạng 2</Text>
          </View>
          
          {/* Hạng 1 (Nằm giữa và cao nhất) */}
          <View style={[styles.podiumItem, styles.podiumRank1]}>
            <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=User&backgroundColor=e6e6e6' }} style={styles.avatarLarge} />
            <Text style={styles.podiumName}>User</Text>
            <Text style={styles.podiumRankText}>Hạng 1</Text>
          </View>

          {/* Hạng 3 */}
          <View style={[styles.podiumItem, styles.podiumRank3]}>
            <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=Hung&backgroundColor=e6e6e6' }} style={styles.avatar} />
            <Text style={styles.podiumName}>Hùng</Text>
            <Text style={styles.podiumRankText}>Hạng 3</Text>
          </View>
        </View>
        ...
        <View style={styles.highlightCard}>
          <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=Dao&backgroundColor=f2d1ce' }} style={styles.highlightAvatar} />
          <Text style={styles.highlightText}>
            <Text style={{ fontWeight: 'bold' }}>Đào</Text> vừa hoàn thành xong 4 chu kỳ
          </Text>
        </View>

        <View style={styles.highlightCard}>
          <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=Dung&backgroundColor=f3ebe9' }} style={styles.highlightAvatar} />
          <Text style={styles.highlightText}>
            <Text style={{ fontWeight: 'bold' }}>Dũng</Text> vừa hoàn thành xong 2 chu kỳ
          </Text>
        </View>

        <View style={styles.highlightCard}>
          <Image source={{ uri: 'https://api.dicebear.com/7.x/notionists/png?seed=Nhat&backgroundColor=67bdb7' }} style={styles.highlightAvatar} />
          <Text style={styles.highlightText}>
            <Text style={{ fontWeight: 'bold' }}>Nhật</Text> vừa hoàn thành xong 5 chu kỳ
          </Text>
        </View>

        {/* Đẩy lề dưới để không bị che bởi Bottom Bar */}
        ...
        <Animated.View style={[
          ...
          <Svg width={110} height={60} viewBox="0 0 110 60">
            <Path 
              ...
              fill="#F3EBE9" // Khớp với màu nền Leaderboard
            />
          </Svg>
        </Animated.View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F3EBE9' }, // Màu nền kem y hệt thiết kế
  header: { 
    flexDirection: 'row', alignItems: 'center', 
    paddingTop: 60, paddingBottom: 15, paddingHorizontal: 20
  },
  backButton: { marginRight: 15 },
  headerTitle: { fontSize: 22, fontWeight: '900', color: '#2A3B38' },
  divider: { height: 1, backgroundColor: '#D6CBC9', width: '100%' },
  ...
  toggleActive: { backgroundColor: '#67BDB7' },
  toggleText: { fontSize: 16, fontWeight: 'bold', color: '#2A3B38' },
  ...
  podiumRank2: {
    width: 95, height: 150, 
    backgroundColor: '#3D544C', 
    borderTopLeftRadius: 20, borderBottomLeftRadius: 20,
    marginRight: -15, // Tạo độ lún đè vào bục 1
    zIndex: 1,
  },
  podiumRank3: {
    width: 95, height: 130, 
    ...
    marginLeft: -15, // Tạo độ lún đè vào bục 1
    zIndex: 1,
  },
  avatar: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#FFF', marginBottom: 10 },
  avatarLarge: { width: 66, height: 66, borderRadius: 33, backgroundColor: '#FFF', marginBottom: 15 },
  podiumName: { color: '#FFF', fontWeight: 'bold', fontSize: 16 },
  podiumRankText: { color: '#D6CBC9', fontSize: 12, marginTop: 2 },

  // HIGHLIGHT
  highlightTitle: { fontSize: 18, fontWeight: 'bold', color: '#2A3B38', marginBottom: 15 },
  highlightCard: {
    backgroundColor: '#FFF',
    borderRadius: 15,
    padding: 15,
  ...
  highlightAvatar: { width: 44, height: 44, borderRadius: 22, marginRight: 15 },
  highlightText: { fontSize: 15, color: '#2A3B38', flex: 1 },

  // --- BOTTOM BAR ---
  bottomBarWrapper: { position: 'absolute', bottom: 0, width: '100%', height: 100 },
  bottomBar: { position: 'absolute', bottom: 0, width: '100%', height: 70, backgroundColor: '#F2D1CE', borderTopLeftRadius: 30, borderTopRightRadius: 30 },
  cutoutMask: {
    position: 'absolute', bottom: 35, left: 0, 
    width: MASK_SIZE, height: MASK_SIZE, borderRadius: MASK_SIZE / 2, 
    backgroundColor: '#F3EBE9', zIndex: 1,
  },
  iconsContainer: { position: 'absolute', bottom: 0, width: '100%', height: 70, flexDirection: 'row', paddingHorizontal: PADDING_HORIZONTAL, zIndex: 2 },
